...
import airbnbkgb.config.kafka.KafkaProcessor;
...
public class ViewPageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenRoomEdited_then_UPDATE_1(@Payload RoomEdited roomEdited) {
        try {
            if (!roomEdited.validate()) return;
            // view 객체 조회
            ...
            if (viewPageOptional.isPresent()) {
                ...
                viewPage.setName(roomEdited.getName());
                viewPage.setDescription(roomEdited.getDescription());
                viewPage.setPrice(roomEdited.getPrice());
                // view 레파지 토리에 save
                viewPageRepository.save(viewPage);
            }
        } catch (Exception e) {
    ...
    public void whenRoomCancelled_then_UPDATE_4(
        ...
        try {
            ...
            if (viewPageOptional.isPresent()) {
                ViewPage viewPage = viewPageOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                viewPage.setRoomStatus(cancelled);
                // view 레파지 토리에 save
                viewPageRepository.save(viewPage);
            }
        } catch (Exception e) {
